...
${body}`;
  ...
  var CAPTURE_PLAYBACK_RATES = Object.freeze([16, 12, 8, 4, 2, 1]);
  ...
  function armPlayerOutputCanary(evidence = {}) {
    pendingCanaryEvidence = {
      ...
      manifestUrl: typeof evidence.manifestUrl === "string" ? evidence.manifestUrl : null,
      keyFormats: Array.isArray(evidence.keyFormats) ? evidence.keyFormats.filter((value) => typeof value === "string").slice(0, 4) : [],
      armedAt: Date.now(),
      expiresAt: Date.now() + CANARY_ARM_MS
    };
    for (const store of canaryStores) store.evidence = pendingCanaryEvidence;
  }
  function readPlayerOutputCanary() {
    const stores = [...canaryStores].filter((store) => store.evidence?.expiresAt > Date.now()).sort((left, right) => right.totalBytes - left.totalBytes);
    const tracks = [];
  ...
  function startCaptureAcceleration(store) {
    ...
    const tick = () => {
      ...
      const now = Date.now();
      const queuePressure = store.fullCapture.queuedBytes / MAX_CAPTURE_QUEUE_BYTES;
      const mayShift = now - acceleration.lastShiftAt >= 2e3;
      if (mayShift && now - acceleration.startedAt > 2500 && (acceleration.stalled || queuePressure >= 0.7)) {
      ...
      } else if (mayShift && !acceleration.stalled && queuePressure < 0.25 && acceleration.stableTicks >= 20 && acceleration.rateIndex > 0) {
        acceleration.rateIndex -= 1;
        acceleration.lastShiftAt = now;
        acceleration.stableTicks = 0;
  ...
  function classifyAppendedMediaBuffer(value) {
    let bytes;
    if (value instanceof ArrayBuffer) bytes = new Uint8Array(value);
    else if (ArrayBuffer.isView(value))
      bytes = new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
  ...
  function isCustomProtectedPlayerOutputCandidate(probe) {
    ...
    const formats = Array.isArray(probe.encryptionKeyFormats) ? probe.encryptionKeyFormats : [];
  ...
  function normalizeFormatDescriptor(format, { muxed = false } = {}) {
    ...
    const audioMetadata = type === "audio" ? normalizeYouTubeAudioMetadata(format) : {};
    return {
      id: type === "audio" && audioMetadata.audioTrackId ? `youtube-audio-${itag}-${stableTrackToken(audioMetadata.audioTrackId)}` : `youtube-${type}-${itag}`,
      type,
      itag,
      sourceUrl: null,
  ...
  function normalizeYouTubeAudioMetadata(format) {
    ...
    const contentRole = safeToken2(xtags.get("acont"), 40);
    const audioRole = contentRole === "original" ? "original" : contentRole === "dubbed" ? "dubbed" : contentRole === "dubbed-auto" ? "auto_dubbed" : contentRole === "descriptive" ? "descriptive" : contentRole === "secondary" ? "secondary" : null;
    return {
      language: safeToken2(xtags.get("lang"), 40) || languageFromAudioTrackId(audioTrack?.id),
      audioTrackId: safeToken2(audioTrack?.id, 240),
  ...
  function decodeYouTubeXtags(value) {
    ...
    if (typeof value !== "string" || !value || value.length > 4096) return tags;
    try {
