...
export function selectVisibleMediaItems(items = [], maximum = 8) {
  ...
  for (const item of sorted) {
    ...
    if (item.kind === "direct") {
      const key = directMediaGroupKey(item);
...
function prioritizeFacebookPlayback(items) {
  ...
  const preferred = playing.length
    ? playing
    : visible.length
      ? visible
      : [...facebookItems]
          .sort(
            (left, right) =>
              (right.lastSeenAt || right.firstSeenAt || 0) -
              (left.lastSeenAt || left.firstSeenAt || 0),
          )
          .slice(0, 1);
...
function facebookPageVideoId(value) {
  try {
    ...
    if (!(page.hostname === "facebook.com" || page.hostname.endsWith(".facebook.com")))
      return null;
    return (
      page.searchParams.get("v") ||
      page.pathname.match(/\/(?:reel|videos)\/(\d+)/i)?.[1] ||
      null
    );
  } catch {
    return null;
...
function facebookQualityLabel(item) {
  const tag = facebookEncodingTag(item.sourceUrl);
  const height =
    item.resolution?.height || Number(tag?.match(/(?:^|[^0-9])(\d{3,4})p/i)?.[1]);
...
function facebookEncodingTag(value) {
  try {
    ...
    return typeof envelope?.vencode_tag === "string"
      ? envelope.vencode_tag
      : null;
  } catch {
    return null;
  }
...
export function formatMediaName(item) {
  ...
  try {
    ...
    if (item.kind === "direct" && isFacebookCdnHost(url.hostname)) {
      ...
      return shortName ? `Facebook video · ${shortName}` : "Facebook video";
    }
...
function directMediaGroupKey(item) {
  ...
  try {
    ...
    for (const name of ["range", "bytestart", "byteend"])
      url.searchParams.delete(name);
    const entries = [...url.searchParams.entries()].sort(([leftKey, left], [rightKey, right]) =>
      leftKey.localeCompare(rightKey) || left.localeCompare(right),
    );
    url.search = "";
